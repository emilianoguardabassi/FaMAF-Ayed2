...
static void move(unsigned int current, hanoi_t hanoi, stack *source_ptr,
                 stack *target_ptr, stack *aux_ptr);
...
hanoi_t hanoi_init(unsigned int disk_count) {
  ...
  hanoi->target = stack_empty(); // targer was NULL insted of an empty stack.
  hanoi->disk_count = disk_count;
  hanoi->source = stack_empty(); // add line, source was not initialized
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
  assert(hanoi != NULL);
  stack_destroy(hanoi->source); // free stack type arguments of hanoi struct
  stack_destroy(hanoi->aux);    // free stack type arguments of hanoi struct
  stack_destroy(hanoi->target); // free stack type arguments of hanoi struct
  free(hanoi);
  return NULL;
}
...
static void print_disks(stack_elem *arr_a, unsigned int length_a,
                        stack_elem *arr_b, unsigned int length_b,
                        stack_elem *arr_c, unsigned int length_c, int position,
                        int width) {
  if (position >= 0) {
    ...
    printf("%*s%s%*s%*s%s%*s%*s%s%*s\n", padlen1, "", value_a, padlen1, "",
           padlen2, "", value_b, padlen2, "", padlen3, "", value_c, padlen3,
           "");
    free(value_a);
    free(value_b);
    free(value_c);
    print_disks(arr_a, length_a, arr_b, length_b, arr_c, length_c, position - 1,
                width);
  } else {
    int padlen = (width - 1) / 2;
    printf("%*s%s%*s%*s%s%*s%*s%s%*s\n", padlen, "", "S", padlen, "", padlen,
           "", "M", padlen, "", padlen, "", "T", padlen, "");
  }
}
